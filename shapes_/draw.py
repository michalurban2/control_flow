...
from shapes_.shapes import Group, Circle, Rectangle, Polygon
...
@singledispatch
def draw(shape):
    raise TypeError(f"Can't draw {shape!r} ")
...
def _(ractangle):
    return (
        ...
        f'x="{ractangle.p[0]}" '
        f'y="{ractangle.p[1]}" '
        f'width="{ractangle.width}" '
        f'height="{ractangle.height}" '
        f'{ractangle.attrs()} />'
    )
...
#     except KeyError:
#         print(f"Cant't draw {shape!r}")
#     else:
#         drawer(shape)
#
#     # if isinstance(shape, Rectangle):
#     #     draw_ractangle(shape)
...
#     #     draw_group(shape)
#     # else:
#     #     raise TypeError(f"Cant't draw {shape!r} ")
